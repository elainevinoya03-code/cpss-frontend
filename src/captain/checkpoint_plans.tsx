...
import {
  ...
  selectCls,
  textareaCls,
} from "../chief_tanod/patrolUi";
import { IncidentDetailsModal } from "../chief_tanod/patrolAnalysisView";
import { ApprovalModal, CheckpointPlansView, PlanDetailModal } from "../chief_tanod/patrolPlansView";
import { removeCheckpointPlan, upsertCheckpointPlan, useCheckpointPlans } from "../chief_tanod/checkpointPlanStore";

/* --------------------------------------------------------------------- */
/* Checkpoint Plans — Captain page                                        */
/* --------------------------------------------------------------------- */

export default function CheckpointPlans() {
  ...
  const [approveTarget, setApproveTarget] = useState<CheckpointPlan | null>(null);
  ...
  /* ---------------- Derived incident sets (planner overlay) ---------------- */
  ...
  function commitMapPoint(lat: number, lng: number) {
    ...
    if (mapMode === "set_fixed") {
      next = [
        { id: nextPointId(), kind: "fixed", label: "FIXED", name: `${draft.targetArea || "Checkpoint"} post`, address: "", landmark: "", description: "", remarks: "", lat, lng },
      ];
      msg = `Fixed location set at (${lat.toFixed(0)}, ${lng.toFixed(0)})`;
    } else if (mapMode === "set_start") {
      next = next.filter((p) => p.kind !== "start");
      next.push({ id: nextPointId(), kind: "start", label: "A", name: "Point A", address: "", landmark: "", description: "Starting point of route", remarks: "", lat, lng });
      msg = `Point A set at (${lat.toFixed(0)}, ${lng.toFixed(0)})`;
    } else if (mapMode === "set_end") {
      next = next.filter((p) => p.kind !== "end");
      next.push({ id: nextPointId(), kind: "end", label: "B", name: "Point B", address: "", landmark: "", description: "End point of route", remarks: "", lat, lng });
      msg = `Point B set at (${lat.toFixed(0)}, ${lng.toFixed(0)})`;
    } else if (mapMode === "set_intermediate") {
      const near = nearestRouteInfo(lat, lng);
      if (near) {
        if (near.rid === "primary") {
          const updated = insertSeriesPoint(sortRoutePoints(draft.points), near.anchorId, lat, lng);
          updateDraft({ points: relabelIntermediates(updated) });
          msg = `Checkpoint added to Route 1 at (${lat.toFixed(0)}, ${lng.toFixed(0)})`;
        } else {
          updateDraft({
            routes: (draft.routes ?? []).map((r) => {
              ...
              const series = insertSeriesPoint([start, ...r.points, end], near.anchorId, lat, lng);
              return { ...r, points: relabelIntermediates(series.slice(1, -1)) };
            }),
          });
          const rl = (draft.routes ?? []).find((r) => r.id === near.rid)?.label ?? "route";
          msg = `Checkpoint added to ${rl} at (${lat.toFixed(0)}, ${lng.toFixed(0)})`;
        }
        setMapMode("view");
        flash(msg);
        return;
      ...
      next.push({ id: nextPointId(), kind: "intermediate", label: `CP${cnt}`, name: `Checkpoint ${cnt}`, address: "", landmark: "", description: "", remarks: "", lat, lng });
      msg = `Checkpoint ${cnt} added at (${lat.toFixed(0)}, ${lng.toFixed(0)})`;
    } else if (mapMode === "set_custom") {
      const pt: CpPoint = { id: nextPointId(), kind: "intermediate", label: "CP", name: "", address: "", landmark: "", description: "", remarks: "", lat, lng };
      if (!customTargetId) {
    ...
    setMapMode("view");
    flash(msg || (fresh ? `Point ${fresh.label} set at (${fresh.lat.toFixed(0)}, ${fresh.lng.toFixed(0)})` : "Point set on map"));
  }
  ...
  function submitForApproval() {
    ...
    flash(`Plan ${submittedPlan.code} submitted for approval — notification sent to Punong Barangay`);
  }
  ...
  function decidePlan(planId: string, decision: "approve" | "revision" | "reject", comment: string) {
    const now = new Date().toISOString();
    const current = plans.find((p) => p.id === planId);
    if (current) {
      if (decision === "approve") {
        upsertCheckpointPlan({ ...current, status: "approved", decidedBy: "Punong Barangay", decidedAt: now, approvalComments: comment, revisionComment: undefined, rejectionReason: undefined });
      } else if (decision === "revision") {
        upsertCheckpointPlan({ ...current, status: "revision_required", decidedBy: "Punong Barangay", decidedAt: now, revisionComment: comment, rejectionReason: undefined });
      } else {
        upsertCheckpointPlan({ ...current, status: "rejected", decidedBy: "Punong Barangay", decidedAt: now, rejectionReason: comment, revisionComment: undefined });
      }
    }
    setApproveTarget(null);
    flash(
      decision === "approve"
        ? `Plan ${planId} approved and finalized — ready for deployment`
        : decision === "revision"
          ? `Plan ${planId} returned for revision with comments`
          : `Plan ${planId} rejected with reason`
    );
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900">
      <main className="flex-1 overflow-y-auto px-3 py-4 sm:px-6 sm:py-6">
        <header className="mb-5 border-b border-stone-200 pb-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h1 className="text-2xl font-bold text-stone-900">Checkpoint Plans</h1>
              <p className="mt-1 text-sm text-stone-500">
                Checkpoint Planning Module — draft, schedule and validate checkpoints, then review and decide on plans pending approval
              </p>
            </div>
            {!plannerOpen && (
              <button
                onClick={startManual}
                className="flex h-9 items-center gap-1.5 rounded-lg bg-gradient-to-r from-blue-600 to-indigo-600 px-4 text-[12px] font-semibold text-white shadow-md shadow-blue-500/20 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-blue-500/40"
              >
                <Plus size={14} />
            ...
            {plannerOpen && (
              <button
                ...
                className="flex h-9 items-center gap-1.5 rounded-lg border border-stone-200/50 bg-white/80 px-4 text-[12px] font-medium text-stone-600 backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:bg-white hover:shadow-md"
              >
                <X size={14} />
                Exit Planner
              </button>
        ...
        {plannerOpen && draft ? (
          ...
          <div>
            ...
            <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
              ...
              <button
                onClick={saveAsDraft}
                className="flex items-center gap-1.5 rounded-lg border border-stone-200/50 bg-white/80 px-3 py-1.5 text-[10px] font-semibold text-stone-600 backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:bg-white hover:shadow-sm"
              >
                      ...
                      <div className="rounded-xl border border-teal-200 bg-teal-50/50 p-3">
                        ...
                        <div className="mb-3 grid grid-cols-1 gap-2 sm:grid-cols-4">
                          <button
                            ...
                            className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[10px] font-bold transition ${
                              mapMode === "set_start" ? "bg-green-600 text-white" : "bg-white text-green-700 shadow-sm hover:bg-green-100"
                            }`}
                          >
                            <MapPin size={12} />
                          ...
                          <button
                            ...
                            className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[10px] font-bold transition ${
                              mapMode === "set_end" ? "bg-rose-600 text-white" : "bg-white text-rose-700 shadow-sm hover:bg-rose-100"
                            }`}
                          >
                            <MapPin size={12} />
                          ...
                          <button
                            ...
                            className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[10px] font-bold transition ${
                              mapMode === "set_intermediate" ? "bg-sky-600 text-white" : "bg-white text-sky-700 shadow-sm hover:bg-sky-100"
                            }`}
                          ...
                          <button
                            ...
                            className={`flex items-center justify-center gap-1.5 rounded-lg px-3 py-2 text-[10px] font-bold transition disabled:cursor-not-allowed disabled:opacity-40 ${
                              mapMode === "set_custom" ? "bg-amber-600 text-white" : "bg-white text-amber-700 shadow-sm hover:bg-amber-100"
                            }`}
                          >
                            <Route size={12} />
                              ...
                              onClick={() =>
                                updateDraft({
                                  schedule: {
                                    ...
                                    recurringDays:
                                      o.key === "specific_days" ? draft.schedule.recurringDays : draft.schedule.recurringDays,
                                  },
                                })
                              }
                              className={`rounded-lg border px-2 py-1.5 text-[9px] font-semibold transition ${
                                draft.schedule.recurring === o.key
                              ...
                              return (
                                <button
                                  ...
                                  className={`flex h-8 w-8 items-center justify-center rounded-full text-[10px] font-bold transition ${
                                    on ? "bg-[#0038A8] text-white" : "bg-stone-100 text-stone-500 hover:bg-stone-200"
                                  }`}
                                >
                                  {d[0]}
                                </button>
                ...
                {step === 6 && (
                  <div className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
                    ...
                    <div className="space-y-3">
                      ...
                      {draft.notes.general || draft.notes.safety || draft.notes.equipment || draft.notes.coordination || draft.notes.special || draft.notes.other ? (
                        <div className="rounded-lg border border-stone-100 p-3">
                    ...
                    {atStep(6).length === 0 && (
                      <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2.5 text-[10px] font-semibold text-emerald-700">
                        <CheckCircle2 size={12} className="mr-1 inline" /> Plan is complete — you can submit it for approval.
                      </div>
                    )}
                  </div>
                )}
      ...
      {approveTarget && (
        <ApprovalModal
          ...
          onDecide={(decision, comment) => decidePlan(approveTarget.id, decision, comment)}
        />
      )}
      ...
      {submitted && (
        <ConfirmModal
          ...
          message={`${submitted.code} — ${submitted.name} moved from Draft to Pending Approval. A notification has been sent to the Punong Barangay / designated approver.`}
